...
- Add recursive option to working_copy (2017-10-08)
...
def working_copy(
    ...
    :param recursive: if ``True`` add --recursive, which clones repos
    :type recursive: bool
    ...
    if is_dir(path, use_sudo=use_sudo):
        ...
        git.clone(
            remote_url, path=path, use_sudo=use_sudo, user=user, recursive=recursive
        )
        git.checkout(path=path, branch=branch, use_sudo=use_sudo, user=user)
