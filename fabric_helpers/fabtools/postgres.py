...
- use pg_roles to check for user_exists instead of pg_users
...
def user_exists(name):
    ...
    with settings(hide('running', 'stdout', 'stderr', 'warnings'), warn_only=True):
        res = _run_as_pg(
            ...
            """FROM pg_roles WHERE rolname='%(name)s';\"""" % locals()
        )
    # https://github.com/fabtools/fabtools/issues/4#issuecomment-8845152
    print('user_exists: %s' % res)
    return '1' in res
...
def create_user(
    ...
    Example::
        ...
        if not fabtools.postgres.user_exists('dbuser'):
            fabtools.postgres.create_user('dbuser', password='somerandomstring')  # NOQA
...
def create_database(
    name, owner, template='template0', encoding='UTF8', locale='en_US.UTF-8'
):
    """
...
def create_schema(name, database, owner=None):
    ...
    if owner:
        _run_as_pg(
            """psql %(database)s -c \"CREATE SCHEMA %(name)s """
            """AUTHORIZATION %(owner)s\"""" % locals()
        )
    else:
