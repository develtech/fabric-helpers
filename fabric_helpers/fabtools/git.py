...
- Add recursive submodule option for pull (2017-10-09)
- Add recursive option to clone (2017-10-08)
...
def clone(remote_url, path=None, use_sudo=False, recursive=True, user=None):
    """
    ...
    :param recursive: If ``True`` use --recursive, downloading submodules
    :type recursive: bool
...
def add_remote(path, name, remote_url, use_sudo=False, user=None, fetch=True):
    ...
    if fetch:
        cmd = f'git remote add -f {name} {remote_url}'
    else:
        cmd = f'git remote add {name} {remote_url}'
...
def checkout(path, branch='master', use_sudo=False, user=None, force=False):
    ...
    cmd = f'git checkout {branch} {options}'
